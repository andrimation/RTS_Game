...
class MoveQueueManager():
    ...
    # Zrobić żeby - ok czyli problem jest też tu że za szybko się wywołują obliczenia po sobie - zanim zostaną obliczone - jak tu przerwie obliczenie
    # to ucieka ta lista used cords i w ten sposób ( w rezultacie ) kilka jednostek - coś jest zjebane
    # no coś jest zjebane z wyszukiwaniem wolnego miejsca po wydaniu rozkazu - bo często obiekty dodstają te samo miejsce -
    # funkcję znajdującą miejsce wywołuję przy kliknięciu z rozkazem i tutaj w movemanagerze
    def compute_paths_for_orders(self):
        ...
        for order_destination in self.root.orders_destinations:
            ...
            if isinstance(unitInMove,GameUnit.Tank):
                print(order_destination)
                print(matrixDestination)
    ...
    def execute_units_movement(self):
        ...
        for order in self.root.move_queue:
            ...
            # if isinstance(unitInMove,GameUnit.Tank):
            #     print(order)
            #     print(unitInMove.matrixPosition)
            if refreshMinimap:
